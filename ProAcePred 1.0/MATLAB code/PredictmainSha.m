function [AllData,rr,cc]=PredictmainSha(proteinsequence,Specificity)
proteinsequence=char(proteinsequence);
R=size(proteinsequence,1);
filepath='C:\DataSet\PAPred\Shamenshijun';
nn=0;
LL=17;  %sequence fragment size
ww=[];AllData=[];
for jj=1:R
...
for ii=1:nn
    ...
    clear X;clear Data;
    if ii<nn
        protein=proteinsequence(ww(ii):ww(ii+1)-1,:);
    else
        protein=proteinsequence(ww(nn):R,:);
    end
